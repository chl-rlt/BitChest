<template>
    <table class="border-collapse w-full">
        ...
        <tbody>
            ...
            class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                <row-link :href="route('users.show',user)" className="w-full lg:w-auto text-gray-800 text-center border border-b block lg:table-cell relative lg:static"> {{ user.name }} </row-link>
                ...
                <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                    <div class="flex flex-row justify-center text-center ">
                        <Link class="flex-1 w-1/4 ml-12	" :href="route('users.edit', user)" title="Modify">
    ...
    <div class="inline-block mr-2 mt-2">
        <Link :href="route('users.create')"><button type="button" class="focus:outline-none text-white text-base font-semibold py-2.5 px-10 border-b-4 border-blue-600 rounded-md bg-blue-500 hover:bg-blue-400 mt-6">Create</button></Link>
    </div>
